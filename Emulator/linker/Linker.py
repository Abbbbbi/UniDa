...
class Linker:
    ...
    def load_library(self, soName):
        ...
        for relName in reader.rels:
            for rel in reader.rels[relName]:
                ...
                if r_type == 0:
                    logger.debug("Unhandled relocation type " + str(r_type))
                    continue

                symbol = None if r_sym == 0 else reader.dynsymols[r_sym]
                sym_addr = 0 if symbol is None else symbols_resolved[symbol["name"]] if symbol[
                                                                                            "name"] in symbols_resolved else 0
                ...
                if r_type == ElfConst.R_ARM_ABS32:
                ...
                elif r_type == ElfConst.R_AARCH64_ABS64:
                    ...
                    self.emulator.mu.mem_write(reloc, sym_addr.to_bytes(4, byteorder='little'))
                elif r_type in (ElfConst.R_AARCH64_GLOB_DAT, ElfConst.R_AARCH64_JUMP_SLOT):
                    value = sym_addr + r_addend
                    self.emulator.mu.mem_write(reloc, value.to_bytes(8, byteorder='little'))
                elif r_type == ElfConst.R_ARM_RELATIVE:
                    reloc_bytes = self.emulator.mu.mem_read(reloc, 4)
                    reloc_value = int.from_bytes(reloc_bytes, byteorder='little')
                    value = reader.load_start + reloc_value
                    self.emulator.mu.mem_write(reloc, value.to_bytes(4, byteorder='little'))
                elif r_type == ElfConst.R_AARCH64_RELATIVE:
                    value = reader.load_start + r_addend
                    self.emulator.mu.mem_write(reloc, value.to_bytes(8, byteorder='little'))
                else:
                    logger.debug("Unhandled relocation type %i." % r_type)

        if reader.init_off != 0:
            init_array.append(reader.init_off)
        ...
        logger.debug("Load Done %s" % soName)
        return module
        ...
        if symbol['st_shndx'] == ElfConst.SHN_UNDEF:
            ...
            if target is None:
                if symbol['st_info_bind'] == ElfConst.STB_WEAK:
                    ...
                    logger.debug('=> Undefined external symbol: %s' % name)
                    return None
            else:
                return target
    ...
    def findModuleByAddress(self, addr):
